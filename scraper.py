...
def scrape_date_range(
    ...
    while current <= end:
        day_num += 1
        log.info("  [%d/%d] %s …", day_num, days, current.isoformat())
        try:
...
def enrich_permits(
    ...
    conn.commit()
    log.info("  Permits stored: %d", stored)
    return stored
...
def enrich_sub_records(
    ...
    conn.commit()
    log.info("  Sub-record enrichment complete")
...
def main():
    ...
    if not args.skip_inspections:
        ...
        total = scrape_date_range(client, conn, start_date, end_date)
    ...
    if not args.skip_enrich:
        ...
        enrich_permits(client, conn, limit=args.enrich_limit)
        client.delay()

        log.info("")
        log.info("Phase 3: Enriching sub-records (fees, contacts)")
        enrich_sub_records(client, conn, limit=args.enrich_limit)

    # Summary
    log.info("")
    log.info("=" * 60)
    log.info("SCRAPE COMPLETE")
    for table in ("inspections", "permits", "fees", "contacts"):
